...
`internal/adminui` 한 기능(카드·페이지·차트·수정)은 `.omc/skills/adminui-cycle/SKILL.md`의 6단계 루프(구현→컨테이너 검증→Playwright 실측→분리 리뷰→스코프 커밋→테스트 문서화)를 따른다. 이 루프는 두 하위 skill을 위임한다:
- `.omc/skills/adminui-visual-verify/SKILL.md` — Playwright 스크린샷 실측(이미지 재빌드·다폭 촬영·판독·실데이터 시드).
- `.omc/skills/test-report/SKILL.md` — 테스트 문서화(아래 R9).
...
**테스트를 실행하면 반드시 문서로 남긴다. 화면(시각/Playwright) 테스트는 캡처 스크린샷을 함께 첨부한다.** 문서 없는 테스트는 미완으로 간주한다.

- 위치: `docs/dev/test-reports/YYYY-MM-DD-<slug>.md`, 자산: `docs/dev/test-reports/assets/<slug>/`.
- 대상: 단위/컨테이너 `go build·vet·test`, live smoke, 시각 검증, 성능·보안 점검.
- 절차·양식: `.omc/skills/test-report/SKILL.md`. 인덱스는 `docs/dev/test-reports/README.md`.
- 실패는 그대로 기록(숨기지 않음) — R2 silent 실패 가시화 원칙과 동일.

**근거:** 사용자 직접 지시 — *"테스트 진행 시에는 반드시 문서로 남겨주세요. 화면 테스트의 경우 캡쳐 화면을 함께 첨부하여 남겨야 합니다."*
