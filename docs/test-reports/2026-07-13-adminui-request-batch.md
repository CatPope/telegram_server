...
## 회차 2 — 파이프라인 색상 재조정 (같은 날 후속)

- **변경:** dispatched의 점선 황색 특별 취급 제거. 그 스타일은 "dev 데이터에서 dispatched가 항상 0"인 현재 상황에 맞춘 것이었고, 실제 운영에서는 평범한 중간 단계이므로(사용자 피드백) **진행 단계 = 파랑 그라데이션(`#1d4ed8`→`#2563eb`→`#3b82f6`), 종착 = 초록** 의미 체계로 통일 — 기존 퍼널 막대·badge 색 의미와 동일.
- **검증:** 이미지 재빌드 후 1440×900 재촬영·판독 — 색 전환 확인, 흰 숫자 대비 정상, 레이아웃 변화 없음. (템플릿 CSS만 변경이라 Go 테스트 영향 없음 — 회차 1 결과 유지.)

![파이프라인 색상 v2](assets/2026-07-13-adminui-request-batch/dash-viewport-1440x900-v2-pipeline-colors.png)
*진행 단계 파랑 그라데이션 → delivered 초록. dispatched 점선 황색 제거.*

- 부수 기록: 검증 중 WSL2 VM(`vmmemWSL`)이 반복 컨테이너 빌드로 62.5GB까지 부풀어 Docker 엔진이 500/502로 열화 → `wsl --shutdown`으로 회수(80GB→19GB) 후 스택 복구·재검증. 재발 방지로 `.wslconfig` 메모리 상한(예: 16GB) 설정 권장.
