...
## 회차 2 — 툴팁 즉시 표시 (같은 날 후속)

- **변경:** 네이티브 SVG `<title>` 툴팁은 브라우저가 ~1초 지연 후 표시(제어 불가, 사용자 피드백) → **CSS hover 즉시 툴팁**으로 교체. 각 점/슬라이스를 `<g class="pt">`로 감싸고 실제 `<text class="tip">` 노드를 `:hover` 시 즉시 표시(paint-order 흰 스트로크로 선 위에서도 판독). 라인차트·스트립·파이·전달추이 4개 차트 적용, 점에는 투명 확대 히트영역(r9~11) 추가. 파이는 hover 시 도넛 중앙 합계를 잠시 숨기고 그 자리에 팁 표시. CSP(zero-JS) 준수 — JS 없이 CSS만.
- **검증:** 전 테스트 green(툴팁 단언을 .tip 텍스트로 갱신). Playwright로 mouse.move 직후(대기 0) 가시성 실측 — 스트립 `dev-admin · 3ms`, 라인차트 `13일 1시 · dev-admin · 0건`, 파이 `missing_bearer · 2건 (40%)` + 중앙 텍스트 hidden 확인.

![파이 슬라이스 hover 즉시 팁](assets/2026-07-14-adminui-r2/pie-hover-1440.png)
*슬라이스 hover 즉시 도넛 중앙에 "missing_bearer · 2건 (40%)" 표시.*

![스트립 점 hover 즉시 팁](assets/2026-07-14-adminui-r2/strip-hover-1440.png)
*스트립 점 hover 즉시 "dev-admin · 3ms" 표시.*
