...
cmd/adminui/         관리자 웹 UI (앱/사용자/키/감사 로그, 127.0.0.1:8081)
cmd/mocktelegram/    mock Telegram Bot API 서버 (dev/test)
cmd/devseed/         dev 시드 헬퍼
internal/adminui/    관리자 UI (세션/CSRF, /admin API 프록시, 키 발급)
...
scripts/             smoke.sh, smoke-adminui.sh, audit-retention.sh, dry-run-rollback.sh
```
...
| [docs/adminui-guide.md](docs/adminui-guide.md) | 관리자 UI 운영 가이드 (기동, 환경변수, 화면별 사용법, 보안) |
