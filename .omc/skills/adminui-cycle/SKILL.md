...
4. 리뷰(분리)  5. 스코프 커밋     6. 테스트 문서화(+스크린샷)
```
...
### 6. 테스트 문서화 — `test-report` skill 위임 (필수)

**테스트를 돌렸으면 반드시 문서로 남긴다. 화면 테스트는 캡처 스크린샷을 함께 첨부한다.** (사용자 영구 지시.) 절차·위치·양식은 `test-report` skill을 따른다:
...
- 관련 규칙: `CLAUDE_ ops.md`(R2 phase 전환, R4 OMC 에이전트, R7 상시, R9 테스트 문서화).
